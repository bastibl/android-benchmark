...
*/
		...
		GR_DEBUG("cl", "OpenCL Error compiling kernel for " << kernelFunctionName);
		GR_DEBUG("cl", "OpenCL Error " << e.err() << ": " << e.what());
		GR_DEBUG("cl", kernelCode);
		if (exitOnFail)
			exit(0);
		else
			return false;
	...
	try {
		...
		GR_DEBUG("cl", "Error getting kernel preferred work group size multiple");
		GR_DEBUG("cl", "OpenCL Error " << e.err() << ": " << e.what());
	}
...
void GRCLBase::cleanup() {
	...
	try {
		...
		sources=NULL;
		GR_DEBUG("cl", "sources delete error.");
	}
	...
	try {
		...
		queue=NULL;
		GR_DEBUG("cl", "queue delete error.");
	}
	...
	try {
		...
		kernel=NULL;
		GR_DEBUG("cl", "Kernel delete error.");
	}
	...
	try {
		...
		program = NULL;
		GR_DEBUG("cl", "program delete error.");
	}
